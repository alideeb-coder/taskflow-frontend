...
export default function Register() {
  ...
  const validate = () => {
    ...
    else if (!/^\S+@\S+\.\S+$/.test(email)) e.email = t('common.invalidEmail');
    if (!password) e.password = t('common.required');
    else if (password.length < 8) e.password = t('common.passwordMin');
    else if (
      !/[A-Z]/.test(password) ||
      !/[a-z]/.test(password) ||
      !/\d/.test(password) ||
      ...
      e.password = t('common.passwordRules');
    }
    if (password !== passwordConfirm) e.password_confirmation = t('common.passwordMatch');
    setErrors(e);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-950 px-4 py-12">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -left-40 top-0 h-96 w-96 rounded-full bg-purple-500/20 blur-3xl" />
        <div className="absolute -right-40 bottom-0 h-96 w-96 rounded-full bg-brand-500/20 blur-3xl" />
      </div>
